...
$("#overlay").addEventListener("change", (e) => {
  ...
  const legends: Record<Overlay, string> = {
    ...
    pressure: "Dark: vented · Purple: compressed · Bursts at 85",
    radiation: "Dark: safe · Green: irradiated · Lead blocks rays",
  };
...
function addToLab(item: PaletteItem) {
  ...
  if (lab.length === 2) {
    ...
    $("#lab-message").textContent = pinnedIngredient
      ? `${results.length ? results.map(itemLabel).join(" + ") : "No reaction"}. ${itemLabel(pinnedIngredient)} locked — right-click another element to try it.`
      : reaction.message;
...
function renderLab() {
  document.querySelectorAll<HTMLElement>("[data-slot]").forEach((slot, i) => {
    slot.innerHTML = lab[i]
      ? `${itemIcon(lab[i], 22)}<span>${itemLabel(lab[i])}</span><button class="pin-ingredient" data-pin="${i}" aria-label="${pinnedIngredient === lab[i] ? "Unpin" : "Pin"} ${itemLabel(lab[i])}" aria-pressed="${pinnedIngredient === lab[i]}" title="${pinnedIngredient === lab[i] ? "Unlock this ingredient" : "Keep this ingredient while trying others"}">${icon(pinnedIngredient === lab[i] ? "lock" : "lock-open")}<span>${pinnedIngredient === lab[i] ? "Locked" : "Lock"}</span></button>`
...
$("#lab-drop").addEventListener("click", (e) => {
  ...
  $("#lab-message").textContent = pinnedIngredient
    ? `${itemLabel(item)} locked. Right-click other elements to try combinations. Left-click still selects your brush.`
